    ...
    <div class="category_count"><?php echo $post_num;?>件中<?php echo $start_count;?>～<?php echo $end_count;?>表示</div>
          <div class="paging">
<?php if($now_page > 1){?>
          <div class="pager-back" location="thread-list" next_no=<?PHP echo ($start_count-1) ?> page=<?php echo $now_page-1 ?> cat_no=<?php echo $_POST['cat_no']?>>
          <a href="javascript:void(0);"> <-前のページへ</a></div>
<?php              }?>
<?php                 if($now_page < $max_page){?>
          <div class="pager-next" location="thread-list" next_no=<?PHP echo ($end_count+1) ?> page=<?php echo $now_page+1 ?> cat_no=<?php echo $_POST['cat_no'] ?>>
