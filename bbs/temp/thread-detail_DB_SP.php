<?php 
    $post_count=0;
    $thr_created = strtotime($output[0]['thr_created']);
...
<div id="dialog"></div>
        ...
        <div class="breadcrumbs">ホーム→<?php echo $val['cat_name'];?>→<?php echo h_escape($val['thr_title']);?></div>
        <div class="thread-item-root thread-detail-root">
          <label class="thread-detail-item"><?php echo h_escape($val['thr_title']);?> </label>
...
<?php if($now_page > 1){?>
          <div class="pager-back" location="detail" next_no=<?PHP echo ($start_count-1) ?> page=<?php echo $now_page-1 ?> thr_no=<?php echo $_POST['thr_no']?>>
          <a href="javascript:void(0);"> <-前のページへ</a></div>
<?php               }//if($now_page > 1){?>
<?php                 if($now_page < $max_page){?>
          <div class="pager-next" location="detail" next_no=<?PHP echo ($start_count+$output_count) ?> page=<?php echo $now_page+1 ?> thr_no=<?php echo $_POST['thr_no'] ?>>
